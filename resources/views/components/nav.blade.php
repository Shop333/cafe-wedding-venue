<nav x-data="{ open: false }" class="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-stone-200">
    <div class="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
        ...
        <div class="hidden md:flex items-center space-x-8 text-[10px] uppercase tracking-[0.2em] font-bold text-stone-500">
            ...
            @auth
                <a href="{{ route('dashboard') }}" class="text-stone-900 bg-stone-100 px-4 py-2 rounded-full hover:bg-stone-200 transition-all">
                    Dashboard
                </a>
            @else
                <a href="{{ route('login') }}" class="hover:text-stone-900 lowercase font-normal opacity-50 hover:opacity-100 transition-all italic">
                    Admin
                </a>
            @endauth
        </div>
    ...
    <div x-show="open" 
        ...
        @auth
            <a href="{{ route('dashboard') }}" class="block text-sm uppercase tracking-widest font-bold text-stone-900">Dashboard</a>
        @else
            <a href="{{ route('login') }}" class="block text-xs italic text-stone-400">Admin Login</a>
        @endauth
